...
void display(void)
{
	...
	float radians = -10 * CDR;
	for (int i = 0; i < N; i++) {
		if (i > 0) normal(vx[i - 1], vy[i - 1], vz[i - 1], vx[i], vy[i], vz[i], wx[i], wy[i], wz[i]);
		wx[i] = vx[i] * cos(radians) + vz[i] * sin(radians);
		wy[i] = vy[i] + 20;
		wz[i] = -vx[i] * sin(radians) + vz[i] * cos(radians);

		glTexCoord2f(0, 0); glVertex3f(vx[i], vy[i], vz[i]);
		glTexCoord2f(0, 0); glVertex3f(wx[i], wy[i], wz[i]);
